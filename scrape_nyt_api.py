...
def get_many(half_days, link, filename, api_key, fq):
    responses = []
    not_scraped = []
    today = datetime(2017, 1, 14)
    yesterday = datetime(2017, 1, 13)
    for day in range(half_days):
        end = str(today).replace('-','')
        half_day = timedelta(hours=24)
        ...
        today = today - half_day
        yesterday = yesterday - half_day
...
if __name__ == '__main__':
    ...
    not_scraped, responses = get_many(half_days=730,link=link,filename=filename,api_key=api_key,fq=fq)
